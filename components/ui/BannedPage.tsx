...
export default function BannedPage() {
  ...
  return <div className='relative min-h-screen px-2.5 items-center justify-center flex w-full'>
    <AnimatePresence mode='wait'>
      {timerEnd>=0
        ...
        : <motion.div key='unban' initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className='relative z-1 p-2.5 flex flex-col rounded-md bg-white dark:bg-[#212121] gap-2.5 border border-neutral-300 dark:border-neutral-700 justify-center items-center lg:p-10 lg:px-20'>
          <p className='text-green-500 dark:text-green-600 font-bold w-full text-center text-lg'>Вы разблокированы</p>
          <p className='text-neutral-800 dark:text-neutral-200 font-medium'>Не нарушайте больше правила :)</p>
          <Link href={'/'} className='bg-green-500 dark:bg-green-600 rounded-md py-1.25 text-center text-white font-medium hover:bg-green-400 dark:hover:bg-green-500 active:bg-green-600 dark:active:bg-green-700 transition-colors duration-300 ease-out px-2.5'>Вернуться на главную</Link>
        </motion.div> }
    </AnimatePresence>
    <AnimatePresence>
      {timerEnd>=0?
        <motion.div key='red' initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className='absolute inset-0 bg-linear-to-t' style={{background:'linear-gradient(#E7000B 1%, transparent 15%, transparent 98%, #E7000B 105%)'}}>

        </motion.div>
        :
        <motion.div key='green' initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className='absolute inset-0 bg-linear-to-t' style={{background:'linear-gradient(#00A63E 1%, transparent 15%, transparent 98%, #00A63E 105%)'}}>
