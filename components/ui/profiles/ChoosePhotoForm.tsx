...
export default function ChoosePhotoForm({avatar, setAvatar}:IPhotoProps) {
  ...
  return<div className='w-full flex flex-col relative'>
    <div className='flex flex-col gap-5'>
      ...
      <p onClick={()=>choosePhoto.setIsHelp(prevState => !prevState)} className='text-[15px] flex flex-row justify-center items-center font-medium text-neutral-500 select-none cursor-pointer dark:text-neutral-400 transition-colors duration-300 ease-out'>Не знаете как сменить?</p>
    </div>
    <AnimatePresence>
      {choosePhoto.isHelp && <motion.div className='overflow-hidden' initial={{height:0}} animate={{height:'auto'}} exit={{height:0}} key={'ListHelp'} >
