...
export default function CardsEditCategory({props, pending, setPending}:{props:ICategories, pending:boolean, setPending:React.TransitionStartFunction}) {
  ...
  const [privateVisible, setPrivateVisible] = React.useState<boolean>(props.visible === 'Admin')
  async function editCategories(){
    setPending(async ()=>{
      if(props.title.trim() === newTitle.trim() && privateVisible === (props.visible === 'Admin')){
        setIsNotify(true)
        setMessage('Ошибка: вы не ввели новое название или не изменили приватность')
        return
      }else {
        const req = await fetch('/api/categories/edit', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({id:props.id, title:newTitle.trim(), privateVisible:privateVisible}),
        })
  ...
  return <div className='flex flex-row justify-between items-center'>
    <input type='text' id={props.id} disabled={pending} defaultValue={props.title} onChange={(e)=>setNewTitle(e.target.value)} className='outline-none border w-[75%] border-neutral-300 dark:border-neutral-700 rounded-md p-1.25 focus:border-neutral-400 dark:focus:border-neutral-600 transition-colors duration-300 ease-out'/>
    <label htmlFor={`private${props.id}`} className='flex items-center justify-center w-8.5 aspect-square cursor-pointer border border-neutral-300 dark:border-neutral-700 rounded-md relative'>
      <input type='checkbox' className='hidden inset-0' id={`private${props.id}`} defaultChecked={props.visible === 'Admin'} onChange={(e)=>setPrivateVisible(e.target.checked)}/>
      {privateVisible && <Lock/>}
    </label>
