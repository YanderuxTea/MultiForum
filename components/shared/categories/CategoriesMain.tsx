...
export default function CategoriesMain({ props }: { props: ICategories }) {
  ...
  const { isAdmin } = useCheckingStaff({
    role: userData ? userData.role : "User",
  });
  const [pending, setPending] = React.useTransition();
  const [openMenu, setOpenMenu] = React.useState(false);
  return (
    <div className="bg-white dark:bg-[#212121] flex-col flex rounded-md border border-neutral-300 dark:border-neutral-700 shadow-md shadow-black/25 dark:shadow-neutral-700/25">
      <AnimatePresence>
        {isAdmin && openMenu && (
      ...
      <div
        className={`transition-colors duration-300 ease-out flex flex-row items-center justify-between font-medium md:text-lg dark:bg-orange-600 rounded-t-md p-2.5 border-neutral-300 dark:border-neutral-700 ${
          open
            ? "border-b text-neutral-900 dark:text-neutral-100"
            : "text-neutral-600 dark:text-neutral-300 rounded-md dark:bg-orange-700"
        }`}
      >
        ...
        <div className="flex flex-row items-center gap-1.25">
          <button
            ...
            className={`transition-all duration-300 ease-out hover:bg-neutral-200 dark:hover:bg-neutral-800 rounded-md cursor-pointer ${
        ...
        {open && (
          <motion.div
            ...
            className={`flex flex-col overflow-clip ${
              props.subCategories.length === 0 && "items-center"
            }`}
          >
            {props.subCategories.length > 0 ? (
              ...
              <p className="text-neutral-700 dark:text-neutral-300 font-medium py-2.5">
                Разделов нет :(
              </p>
            )}
          </motion.div>
