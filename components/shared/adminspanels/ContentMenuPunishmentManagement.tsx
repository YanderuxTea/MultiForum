...
export default function ContentMenuPunishmentManagement({
  banned,
  setPending,
  setUsers,
  users,
  ...
  const activeBans = useMemo(() => {
    return bans.filter((ban) => {
      ...
      return (
        new Date(ban.date).getTime() + ban.time * 60 * 1000 > Date.now() ||
        ban.time === 0
      );
    });
  }, [users]);
  ...
  return (
    <div className="flex flex-col items-center gap-5">
      ...
      <p className="text-neutral-700 dark:text-neutral-300">
        Активные предупреждения: {activeWarnsNumber}/3
      </p>
      <div className="flex flex-col w-full">
        <p
          ...
          className={`mb-2.5 select-none text-center font-medium cursor-pointer transition-colors duration-300 ease-out ${
            isOpenList
        ...
        <AnimatePresence>
          {isOpenList && <OpenListPunishmentHistory />}
        </AnimatePresence>
      </div>
      <SelectorPunishmentUnpunishment
        checked={checked}
