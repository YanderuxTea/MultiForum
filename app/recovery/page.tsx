    ...
    <div className='border max-w-150 gap-2.5 flex flex-col p-2.5 rounded-2xl bg-white dark:bg-[#212121] border-neutral-300 dark:border-neutral-700 w-full'>
      ...
      <AnimatePresence mode='popLayout'>
        {!sendEmail&&<motion.div key={'sendMailRecovery'} className='flex items-center flex-col gap-2.5 overflow-hidden ' layout initial={{scale:0, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0, opacity:0}} transition={{type:'spring', bounce:0.2}}>
          ...
          <button onClick={(e)=>resetPassword(e.currentTarget.id)} disabled={loading} id='sendRecoveryCode' className='bg-blue-500 dark:bg-blue-600 text-white font-medium rounded-md w-full py-1 disabled:bg-gray-500/25 disabled:text-black/25 dark:disabled:text-white/25 dark:disabled:bg-gray-100/25 transition-colors duration-300 ease-in-out cursor-pointer hover:bg-blue-400 dark:hover:bg-blue-500 active:bg-blue-700 dark:active:bg-blue-800'>{loading?'Отправляем код':'Отправить код'}</button>
        </motion.div>}
      </AnimatePresence>
      <AnimatePresence mode='popLayout'>
        {sendEmail&&!confirmCode&&<motion.div key={'confirmCodeRecovery'} layout initial={{scale:0, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0, opacity:0}} transition={{type:'spring', bounce:0.2}} className='gap-2.5 flex flex-col items-center overflow-hidden'>
          ...
          <button onClick={(e)=>resetPassword(e.currentTarget.id)} disabled={loading} id='confirmRecoveryCode' className='bg-blue-500 dark:bg-blue-600 text-white font-medium rounded-md w-full py-1 disabled:bg-gray-500/25 disabled:text-black/25 dark:disabled:text-white/25 dark:disabled:bg-gray-100/25 transition-all duration-300 ease-in-out cursor-pointer hover:bg-blue-400 dark:hover:bg-blue-500 active:bg-blue-700 dark:active:bg-blue-800'>{loading?'Проверяем':'Подтвердить код'}</button>
        </motion.div>}
      </AnimatePresence>
      <AnimatePresence mode='popLayout'>
        {confirmCode&&<motion.div key={'resetPassRecovery'} initial={{scale:0, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0, opacity:0}} transition={{type:'spring', bounce:0.2}} layout className='flex flex-col items-center overflow-hidden gap-2.5'>
          ...
          <button onClick={(e)=>resetPassword(e.currentTarget.id)} disabled={loading} id='resetPassword' className='bg-blue-500 dark:bg-blue-600 text-white font-medium rounded-md w-full py-1 disabled:bg-gray-500/25 disabled:text-black/25 dark:disabled:text-white/25 dark:disabled:bg-gray-100/25 transition-all duration-300 ease-in-out cursor-pointer hover:bg-blue-400 dark:hover:bg-blue-500 active:bg-blue-700 dark:active:bg-blue-800'>{loading?'Меняем пароль':'Сменить пароль'}</button>
        </motion.div>}
      </AnimatePresence>
    </div>
  </main>
