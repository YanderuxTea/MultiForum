...
export async function POST(req: Request) {
  ...
  const check = await prisma.messagesPosts.findUnique({
    where: { id: id },
    select: { Posts: { select: { locked: true } } },
  });
  if (!check || check.Posts.locked) {
    return NextResponse.json({ ok: false, error: "тема закрыта" });
  }
  await prisma.messagesPosts.update({
    where: { id: id },
    data: { text: data },
  });
  const history = await prisma.historyMessage.create({
    data: {
      afterText: data,
