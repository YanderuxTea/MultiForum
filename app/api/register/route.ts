...
const registerSchema = z.object({
  login: z
    .string()
    .trim()
    .min(6, "Логин не может быть меньше 6 символов")
    .max(25, "Логин не может быть больше 25 символов")
    .regex(
      /^[a-zA-Z0-9]+$/,
...
export async function POST(req: Request) {
  ...
  if (!validateData.success) {
    return NextResponse.json(
      { ok: false, error: z.flattenError(validateData.error) },
      { status: 400 },
    );
  }
  ...
  const existingUser = await prisma.users.findFirst({
    where: {
      OR: [{ login: { equals: login, mode: "insensitive" } }, { email: email }],
    },
  });
  if (existingUser) {
    const field =
      existingUser.login.toLowerCase() === login.toLowerCase()
        ? "Логин"
        : "Почта";
