...
export async function POST(req:Request){
    ...
    try{
      ...
      if(user.recoveryCode === validateData.data.code){
        const hashPass = await bcrypt.hash(validateData.data.password, 12)
        await prisma.users.update({where:{email:validateData.data.email},data:{recoveryCode:null, dateRecoveryCode:null, password:hashPass}})
