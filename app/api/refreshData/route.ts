  ...
  if(typeof validateToken === 'object' && validateToken){
    ...
    if(userData){
      ...
      if(isSame){
        ...
        const newToken = generateJWT({id:userData.id, login:userData.login, role:userData.role, email:userData.email, verifyEmail:userData.verification, verifyAdm:userData.verificationAdm, deviceId:dId})
